      ...
      >
            ...
            return (
              <li key={link.href}>
                <Link
                  href={link.href}
                  className={cn(
                    'relative rounded-full px-4 py-2 text-xs font-medium uppercase tracking-[0.15em] transition-colors',
                    active
                      ? 'text-foreground'
        ...
        <div className="flex items-center gap-2">
          <div className="hidden md:block">
            <GlowButton href="/contact" variant="primary" icon={<Sparkles className="h-4 w-4" />}>
              Create Together
            </GlowButton>
          </div>
          <button
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -10 }}
            className="glass absolute inset-x-4 top-20 rounded-3xl border border-border p-4 md:hidden"
          >
              ...
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link
                    ...
                    className={cn(
                      'block rounded-2xl px-4 py-3 text-sm font-medium uppercase tracking-[0.12em]',
                      pathname === link.href
